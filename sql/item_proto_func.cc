...
bool Item_func_protobuf_update_repeated::val_proto(Proto_wrapper *wr)
{
    ...
    List<String> field_path;
    for (uint32 i = 1; i< arg_count - 2; i++)
    {
      String *val, buf2;
	...
	String *val, buf2;
	val = args[arg_count - 2]->val_str(&buf2);
	int index;
	sscanf (val->c_ptr(), "%d", &index);

    if (wr->update_repeated(&field_path, index, args[arg_count - 1]))
    {
      String fpath;
      List_iterator_fast<String> it_path(field_path);
      while (true)
